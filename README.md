...
The packaged 0.33.3 extension is exercised against tend.host's real schema-2 installer in a disposable fixture, including integrity, scoped-storage, uninstall, and file cleanup. Its browser lifecycle is also mounted and unmounted twice. Connected repositories open from an exact-commit, inert content projection instead of the bundled demo. When an author chooses Keep my custom design, Studio displays the repository-owned HTTPS renderer and never substitutes a TEND Sites theme; custom source components and styles remain preserved and are never executed by the panel. Repository inspection, bundled-starter creation, temporary previews, and production publishing use separate narrow tend.host capabilities with durable evidence. Production reuses the exact reviewed artifact, verifies deployment health, DNS, trusted TLS, and public HTTPS before completion, and restores the previous healthy release on failure.
