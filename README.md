...
The packaged 0.32.1 extension is exercised against tend.host's real schema-2 installer in a disposable fixture, including integrity, scoped-storage, uninstall, and file cleanup. Its browser lifecycle is also mounted and unmounted twice. Repository inspection, bundled-starter creation, and temporary previews use separate narrow tend.host capabilities with durable evidence. Ready preview artifacts retain recipe, platform, and SBOM digests for an exact future build-once handoff; response correlation, monotonic state, replay, health, rollback, DNS, and TLS evidence fail closed. Publishing remains visibly non-executing until its separate permanent-app, traffic, and domain authority is approved.
