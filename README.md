...
The packaged 0.33.5 extension is exercised against tend.host's real schema-2 installer and safety scanner in a disposable fixture, including integrity, declared network access, scoped-storage, uninstall, and file cleanup. Its browser lifecycle is also mounted and unmounted twice. Connected custom sites open as content-first workspaces: only collections declared in `tend.site.json` become editable, while the repository's routes, renderer, and design remain untouched. Preview opens the verified live renderer instead of substituting a TEND Sites theme. For a reviewed GitHub posts collection, the explicit Publish post action sends structured fields through `sites.source.write`; the host derives the Markdown path and performs an exact-head, non-force commit without exposing credentials or generic repository access. Network access is explicit and is used for same-origin/data-image packaging plus the HTTPS renderer restricted to the host-verified connected domain. Repository inspection, connected-post publication, bundled-starter creation, temporary previews, and production publishing use separate narrow tend.host capabilities with durable evidence.
...
The package requests scoped extension storage, `files.read`, `sites.source.read`, `sites.source.connect`, and the separately reviewed `sites.source.write` capability. Files access exposes only owner-authorized image-library summaries and opaque image URLs. Source reads expose credential-free shared-provider status, sanitized repository/branch summaries, a bounded inert compatibility report, and exact re-verified selected-source evidence. Source writes accept only a bounded portable site archive for an actor-owned created site; tend.host validates it, projects it through a fixed renderer, and commits it without exposing paths, commands, credentials, or deployment authority. GitHub App and GitLab connection actions remain host-owned; private keys, installation tokens, personal access tokens, provider paths, deployment, and production authority never enter the extension.
