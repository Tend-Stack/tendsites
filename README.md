...
The packaged 0.32.0 extension is exercised against tend.host's real schema-2 installer in a disposable fixture, including integrity, scoped-storage, uninstall, and file cleanup. Its browser lifecycle is also mounted and unmounted twice. Repository inspection, bundled-starter creation, and temporary previews use separate narrow tend.host capabilities with durable evidence. Ready preview artifacts now retain recipe, platform, and SBOM digests for an exact future build-once handoff. Publishing remains visibly non-executing until its separate permanent-app, traffic, and domain authority is approved.
